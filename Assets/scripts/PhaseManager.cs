...
private void ShowLawEnforcementUnits()
{
...
private void ShowFireDepartmentUnits()
{
...
private void ConfigureCivilianForPhase1(GameObject civilian)
{
    Animator animator = civilian.GetComponent<Animator>();
    WaypointMover mover = civilian.GetComponent<WaypointMover>();
...
private void ConfigureCivilianForPhase2(GameObject civilian)
{
    WaypointMover mover = civilian.GetComponent<WaypointMover>();
    if (mover != null) {
        mover.despawnAtLastWaypoint = true;
...
private void SetAllVillainsDiscColor(Color color)
{
    foreach (GameObject villain in allVillains) {
        GameObject disc = villain.transform.GetChild(2).gameObject;
        disc.SetActive(true);
...
private void ConfigureAllWaypoints(bool isMovingForward, bool canLoop)
{
    ...
    foreach (Waypoints wp in waypoints) {
        wp.isMovingForward = isMovingForward;
        wp.canLoop = canLoop;
        wp.enableAll();
    }
}

private void ConfigureVillainMovement(GameObject villain, string waypointsName, bool isRunning, float moveSpeed)
{
    WaypointMover mover = villain.GetComponent<WaypointMover>();
    Waypoints waypoints = GameObject.Find(waypointsName)?.GetComponent<Waypoints>();
    
    if (mover != null && waypoints != null) {
...
private void ConfigureHostageMovement(GameObject hostage, string waypointsName, bool isRunning, float moveSpeed)
{
    WaypointMover mover = hostage.GetComponent<WaypointMover>();
    Waypoints waypoints = GameObject.Find(waypointsName)?.GetComponent<Waypoints>();
...
private void ConfigureEgressMovements(int baseWaypointIndex)
{
    // Configure villain movements for egress
    for (int i = 0; i < 5; i++) {
        ConfigureVillainMovement(allVillains[i], $"Waypoints{baseWaypointIndex + i}", true, 5);
    }
    
    // Configure physician hostage movement
    ConfigureVillainMovement(physicianHostage, $"Waypoints{baseWaypointIndex + 5}", true, 5);
    
    // Configure other hostages movement
    ConfigureHostageMovement(newHostages[0], $"Waypoints{baseWaypointIndex + 6}", true, 5);
    ConfigureHostageMovement(newHostages[1], $"Waypoints{baseWaypointIndex + 7}", true, 5);
}
